			...
			buttons: [
				{
					...
					onClick: () => {
						let data = input.map(({ ID: instrument_id, description, approve }) => ({
							instrument_id,
							approve,
								...
								? auditResult.map((result, key) => {
										return (
											<tr>
												...
												<td valign="top" style={{ fontSize: 12 }}>
													{result.instrument_origin.sub_topic.topic.name}{' '}
												</td>
												<td valign="top" style={{ fontSize: 12 }}>
													{result.instrument_origin.sub_topic.name}{' '}
												</td>
												<td valign="top" style={{ fontSize: 12 }}>
													<div>{result.description ?? '-'} </div>
													{result.evidence_file != null
														? HTMLReactParser(`<a data-ref=${result.evidence_file} href=${encodeURI(
                                            ...
                                            Evidence File 
												...
												<td>
													...
													{Object.keys(input[key]['description']).length > 0 ? (
														<>
															<div className="pt-1">
																<a href="" data-id={result.id} onClick={showModal}>
																	Rejected Detail
																</a>
															</div>
														</>
