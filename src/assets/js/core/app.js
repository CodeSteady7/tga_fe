...
;(function (window, document, $) {
	...
	$(window).on("load", function () {
		...
		$(".dropdown-notification .dropdown-menu, .dropdown-cart .dropdown-menu").on(
			"click",
			function (e) {
				e.stopPropagation()
			}
	...
	$(document).on(
		...
		function (e) {
			...
			if ($(this).hasClass("text-warning")) {
				...
				for (var i = 0; i < arrList.length; i++) {
					if (
						arrList[i].firstChild.dataset.bsOriginalTitle == $(this).parent()[0].innerText
					) {
						arrList[i].remove()
					}
				}
	...
	$(".form-password-toggle .input-group-text").on("click", function (e) {
		...
		if (formPasswordToggleInput.attr("type") === "text") {
			...
			if (feather) {
				formPasswordToggleIcon
					.find("svg")
					.replaceWith(feather.icons["eye-off"].toSvg({ class: "font-small-4" }))
			}
		}
	})
	...
	$(window).on("scroll", function () {
		...
		if ($body.hasClass("navbar-static")) {
			...
			if (scroll > 65) {
				...
				$(".dark-layout .horizontal-menu .horizontal-menu-wrapper.header-navbar").css(
					"background",
					"#161d31"
				)
			} else {
