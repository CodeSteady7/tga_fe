...
const submitfulfillment = async (auditID, InstrumentID, { description, file }) => {
	return axios.postForm(
		`${process.env.REACT_APP_API_URL}audits/${auditID}/instrument/${InstrumentID}/fulfillment`,
...
const finishFulFillment = async auditID => {
	return axios.put(
		`${process.env.REACT_APP_API_URL}audits/${auditID}/finish/fulfillment`,
		{},
		{
			headers: {
				Authorization: `Bearer ${localStorage.getItem('token')}`,
...
const inputActionPlan = async (ID, { action_plan_description }) => {
	return axios.put(
		`${process.env.REACT_APP_API_URL}audits/rejected/${ID}`,
